...
var toBuckets             = (fish:number[])          : number[] => h.rangee(0,9).map(x => fish.count(x));
...
var addBuckets = (curBucket:number[], addEachBucket:number[][]) : number[] =>  h.rangee(0,9).map(f => addEachBucket[f].times(curBucket[f])).sum0();
